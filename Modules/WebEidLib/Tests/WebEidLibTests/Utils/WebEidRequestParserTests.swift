...
struct WebEidRequestParserTests {
    ...
    @Test
    func parseSignURL_normalisesBase64UrlHashToStandardBase64() throws {
        let hashData = Data(repeating: 0xFB, count: 32)
        let standardBase64 = hashData.base64EncodedString()
        let base64UrlHash = standardBase64
            .replacing("+", with: "-")
            .replacing("/", with: "_")
            .replacing("=", with: "")
        ...
        #expect(base64UrlHash != standardBase64)
        #expect(result.hash == standardBase64)
    }

    @Test
    func parseAuthURL_throwsWithEmptyResponseUriWhenFragmentMissing() throws {
        let url = try #require(URL(string: "https://id.eesti.ee/auth"))

        do {
            _ = try WebEidRequestParser.parseAuthURL(url)
            Issue.record("Expected parseAuthURL to throw")
        } catch let error as WebEidException {
            #expect(error.responseUri.isEmpty)
        }
    }

    @Test
    func parseAuthURL_throwsWithEmptyResponseUriWhenFragmentUndecodable() throws {
        let url = try #require(URL(string: "https://id.eesti.ee/auth#ÖÖÖÖ"))

...
struct WebEidSignServiceTests {
    ...
    @Test
    func buildCertificatePayload_advertisesRSAForAnRSASigningCertificate() async throws {
        let cert = TestCertificateUtil.getSampleCertificate()

        let data = try await service.buildCertificatePayload(signingCert: cert)

        let payload = try #require(try JSONSerialization.jsonObject(with: data) as? [String: Any])
        let algorithms = try #require(payload["supportedSignatureAlgorithms"] as? [[String: Any]])

...
struct WebEidAuthServiceTests {
    ...
    @Test
    func buildAuthToken_includesRSASigningCertificateInsteadOfFailing() async throws {
        let authCert = Data(base64Encoded: testAuthCert) ?? Data()
        let rsaSigningCert = TestCertificateUtil.getSampleCertificate()
        let signature = Data(base64Encoded: testSignature) ?? Data()

        let data = try await service.buildAuthToken(
            authCert: authCert,
            signingCert: rsaSigningCert,
            signature: signature
        )
        ...
        #expect(token["format"] as? String == "web-eid:1.1")

        let certificates = try #require(token["unverifiedSigningCertificates"] as? [[String: Any]])
        let algorithms = try #require(certificates.first?["supportedSignatureAlgorithms"] as? [[String: Any]])
        #expect(algorithms.allSatisfy { $0["cryptoAlgorithm"] as? String == "RSA" })
        #expect(algorithms.allSatisfy { $0["paddingScheme"] as? String == "PKCS1.5" })
    }
}

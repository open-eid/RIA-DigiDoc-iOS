...
struct WebEidAlgorithmUtil: Loggable {
    ...
    static func buildSupportedSignatureAlgorithms(publicKey: SecKey) throws -> [[String: Any]] {
        let descriptor = try algorithmDescriptor(publicKey)

        return supportedHashFunctions.map { hashFunction in
            [
                "cryptoAlgorithm": descriptor.cryptoAlgorithm,
                "hashFunction": hashFunction,
                "paddingScheme": descriptor.paddingScheme
            ]
        }
    }

    private static func algorithmDescriptor(
        _ publicKey: SecKey
    ) throws -> (cryptoAlgorithm: String, paddingScheme: String) {
        if try isEC(publicKey) {
            return ("ECC", "NONE")
        }
        if try isRSA(publicKey) {
            return ("RSA", "PKCS1.5")
        }
    ...
    static func buildSignatureAlgorithm(
        ...
        let normalizedHashFunction = hashFunction.uppercased()

        guard supportedHashFunctions.contains(normalizedHashFunction) else {
        ...
        return [
            "cryptoAlgorithm": descriptor.cryptoAlgorithm,
            "hashFunction": normalizedHashFunction,
            "paddingScheme": descriptor.paddingScheme
        ]
    }

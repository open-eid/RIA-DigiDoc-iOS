...
public actor SignedContainer: Sendable, SignedContainerProtocol {
    ...
    public static func openOrCreate(file: URL, dataFiles: [URL?]?) async throws -> SignedContainer {
        ...
        let isFirstDataFileContainer = isContainer(file: firstFile)

        containerFile = file

        if let dataFiles = dataFiles, dataFiles.count == 1, isFirstDataFileContainer {
            isExistingContainer = true
            return try await open(file: file)
        } else {
            isExistingContainer = false
            return try await create(file: file, dataFiles: dataFiles)
        }
    }

    static func isContainer(file: URL) -> Bool {
        return Constants.Container.containerExtensions.contains( file.pathExtension.lowercased())
    }

...
struct WebEidView: View {
    ...
    var body: some View {
            ...
            if viewModel.authRequest != nil {
                NFCView(
                    ...
                    onSuccessWebEid: {
                        ...
                        Task { await viewModel.setWebEidSessionActive(false) }
                    },
                    onErrorWebEid: {
                        isWebEidAuthenticating = false
                    },
        ...
        .onChange(of: viewModel.errorEventId) { _, _ in
            guard viewModel.errorKey != nil else { return }
            Toast.show(errorMessage)
        }
        .onChange(of: webEidUrl) {_, url in
            handleWebEidOperation(for: url)
        }
        .onChange(of: viewModel.authRequest) {_, newRequest in
            guard newRequest != nil else { return }
            activateWebEidSession()
        }
        .onChange(of: viewModel.certRequest) {_, newRequest in
            guard newRequest != nil else { return }
            activateWebEidSession()
        }
    }

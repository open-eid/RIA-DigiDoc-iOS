    ...
    var body: some View {
        ZStack {
            ...
            InputModal(
                ...
                placeholder: encryptViewModel.containerName,
                text: Binding<String>(
                    get: {
                        URL(fileURLWithPath: encryptViewModel.containerName)
                            .deletingPathExtension()
                            .lastPathComponent
                    },
                    set: { newValue in
                        let existingExtension = URL(fileURLWithPath: encryptViewModel.containerName).pathExtension
                ...
                onConfirm: {
                    ...
                    Task {
                        let uniqueContainerName = await encryptViewModel.renameContainer(to: newContainerName)
                        defer { newContainerName = "" }

                        if let uniqueContainerName {
                            encryptViewModel.containerName = uniqueContainerName.lastPathComponent
                        }
                    }
                },
                onCancel: {

...
final class WebEidViewModelTests {
    ...
    @Test
    func handleAuth_setsLocalErrorAndNoResponseWhenFragmentMissing() throws {
        ...
        #expect(viewModel.errorKey == "Invalid authentication request")
        #expect(viewModel.relyingPartyResponseEvents == nil)
    }

    @Test
    func handleAuth_clearsPreviousCertificateRequest() throws {
        let certURL = try makeWebEidURL(payload: ["responseUri": "https://example.com/cert"])
        let authURL = try makeWebEidURL(payload: [
            "challenge": String(repeating: "A", count: 44),
            "loginUri": "https://example.com/login"
        ])

        viewModel.handleCertificate(url: certURL)
        #expect(viewModel.certRequest != nil)

        viewModel.handleAuth(url: authURL)
    ...
    @Test
    func handleSign_setsLocalErrorAndNoResponseWhenFragmentMissing() throws {
    ...
    @Test
    func handleSign_preservesPreviousRequestWhenTheNewLinkIsMalformed() throws {
        let certURL = try makeWebEidURL(payload: ["responseUri": "https://example.com/cert"])
        let invalidURL = try #require(URL(string: "https://example.com/not-a-valid-sign-request"))

        viewModel.handleCertificate(url: certURL)
        #expect(viewModel.certRequest != nil)

        viewModel.handleSign(url: invalidURL)
    ...
    private func makeWebEidURL(payload: [String: Any]) throws -> URL {
        let jsonData = try JSONSerialization.data(withJSONObject: payload, options: [])
        let fragment = jsonData.base64EncodedString()
        return try #require(URL(string: "web-eid://request#\(fragment)"))
    }

    // MARK: - error signalling and the one-response latch

    @Test
    func handleUnknown_bumpsErrorEventIdEachTimeSoARepeatedFailureStillNotifies() throws {
        let url = try #require(URL(string: "web-eid://unknown"))

        viewModel.handleUnknown(url: url)
        let first = viewModel.errorEventId

        viewModel.handleUnknown(url: url)

        #expect(viewModel.errorKey == "Invalid Web eID request")
        #expect(viewModel.errorEventId == first + 1)
    }

    @Test
    func onlyTheFirstResponseForOneRequestReachesTheRelyingParty() async throws {
        viewModel.certRequest = makeCertificateRequest()

        await viewModel.handleWebEidCertificateResult(signingCert: Data("cert".utf8))
        let firstResponse = try #require(viewModel.relyingPartyResponseEvents)

        viewModel.relyingPartyResponseEvents = nil
        await viewModel.handleUserCancelled()

        #expect(viewModel.relyingPartyResponseEvents == nil)
        #expect(firstResponse.fragment?.isEmpty == false)
    }

    @Test
    func aNewIncomingLinkReArmsTheResponseLatch() async throws {
        viewModel.certRequest = makeCertificateRequest()
        await viewModel.handleWebEidCertificateResult(signingCert: Data("cert".utf8))
        #expect(viewModel.relyingPartyResponseEvents != nil)
        viewModel.relyingPartyResponseEvents = nil
